...
mercury.hook.before("CREATE_BLOG_RECORD", async function (this: any) {
  ...
  try {
    ...
    const existingTitles = blogs.map((blog: { title: string; }) => blog.title.toLowerCase());
    if (existingTitles.includes(title)) {
      throw new Error(`A blog with the title "${title}" already exists. Please choose a different title.`);
    }
