  ...
  return (
    <div className="bg-gray-200">
      <div className=" w-[100%] px-5 bg-gray-200  text-black flex flex-row justify-between items-center py-1">
        ...
        {usePathname().includes("admin") && token &&
          <div className="flex flex-row gap-2 w-[38px] h-[38px] rounded-full border-[1px] border-black" onClick={() => setdropdown(!dropdown)}>
        ...
        {dropdown &&
          <div className="drop-down z-10 w-48 overflow-hidden bg-white rounded-md shadow absolute top-12 right-3">
            <ul>
              <li className="px-3 py-3 text-sm font-medium flex items-center space-x-2 hover:bg-slate-400" onClick={() => { deleteCookie('tokenkey'), router.push('/login'), setdropdown(false) }}>
                <span> Logout </span>
              </li>
